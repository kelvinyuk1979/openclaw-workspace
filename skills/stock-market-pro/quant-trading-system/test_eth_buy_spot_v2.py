#!/usr/bin/env python3
"""
ETH 实盘买入测试 - 现货市价单（使用 quote 货币金额）
"""
...
def main():
    print("=" * 60)
    print("🚀 ETH 实盘买入测试 - 10 USDT (现货市价单)")
    print("=" * 60)
    ...
    print(f"   预计数量：{trade_amount / eth_price:.8f} ETH")
    
    # 3. 执行市价买入（使用 quote 货币金额）
    # OKX 市价买单：sz 表示 quote 货币金额（USDT），当 ordType=market 且 side=buy
    print(f"\n3️⃣ 执行市价买入订单...")
    print(f"   币种：ETH-USDT")
    print(f"   方向：buy")
    print(f"   金额：{trade_amount} USDT")
    print(f"   类型：市价单（quote 计价）")
    
    timestamp = datetime.now(timezone.utc).isoformat(timespec='milliseconds').split('.')[0] + 'Z'
    method = 'POST'
    request_path = '/api/v5/trade/order'
    
    # 关键：市价买单时，sz 表示 quote 货币（USDT）金额
    body = json.dumps({
        'instId': 'ETH-USDT',
        'tdMode': 'cash',
        'side': 'buy',
        'sz': str(trade_amount),  # USDT 金额
        'ordType': 'market',
        'tgtCcy': 'quote_ccy'  # 指定使用 quote 货币计价
    })
    ...
    if result.get('code') == '0' and result.get('data'):
        ...
        if order_result.get('code') == '0' and order_result.get('data'):
            ...
            filled_size = float(order_info.get('accFillSz', 0)) if order_info.get('accFillSz') else 0
            filled_value = float(order_info.get('accFillVal', 0)) if order_info.get('accFillVal') else trade_amount
            
            print(f"   订单状态：{state}")
            print(f"   成交价：${fill_price:.2f}")
            print(f"   成交数量：{filled_size:.8f} ETH")
            print(f"   成交金额：${filled_value:.2f} USDT")
            ...
            record = f"""
### 📈 ETH 实盘买入测试（现货市价） - {timestamp_str} GMT{tz[:3]}:{tz[3:]}
...
"""
            ...
            os.system(f'git commit -m "📈 ETH 实盘买入测试：{filled_size:.8f} ETH @ ${fill_price:.2f}"')
            print(f"   ✅ Git 提交完成")
            ...
            print(f"   金额：${filled_value:.2f} USDT")
